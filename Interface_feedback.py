...
frame_bottom = ctk.CTkFrame(window, width =int(0.9*screen_width), height = int(0.7*screen_height))
frame_bottom.grid(row=1, column = 0, rowspan = 10, sticky = 'nsew', padx = 20, pady = 20)
frame_bottom.grid_propagate(False)

frame_left = ctk.CTkFrame(frame_bottom, width =int(0.45*screen_width), height = int(0.6*screen_height))
frame_left.grid(row=0, column = 0, sticky = 'nsew', padx = 20, pady = 20)
frame_left.grid_propagate(False)

frame_right = ctk.CTkFrame(frame_bottom, width =int(0.45*screen_width), height = int(0.6*screen_height))
...
def start_training():
    ...
    def show_video(cap, new_width, new_height):
        ...
        if not hasattr(show_video, 'canvas1_created'):
            ...
            show_video.canvas1 = ctk.CTkCanvas(frame_left,  width =int(0.45*screen_width), height = int(0.6*screen_height))
    ...
    show_video(video, int(0.67*screen_width), int(0.91*screen_height))
    ...
    text_to_speech("NOW IT'S YOUR TURN")
    ...
    def show_angle_on_live(cap, index1, index2, index3):
        ...
        if not hasattr(show_angle_on_live, 'canvas2_created'):
            ...
            show_angle_on_live.canvas2 = ctk.CTkCanvas(frame_right, width =int(0.45*screen_width), height = int(0.6*screen_height))
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while cap.isOpened():
                ...
                resized_frame = cv2.resize(flipped_frame, (int(0.67*screen_width), int(0.91*screen_height)))
    ...
    show_angle_on_live(cv2.VideoCapture(0), 15, 13, 11)
